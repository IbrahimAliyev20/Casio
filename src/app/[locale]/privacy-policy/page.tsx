...
import { getPrivacyPolicyQuery } from '@/services/pages/queries';

export default function PrivacyPage() {
  const { data: privacyPolicy, isLoading, error } = useQuery(getPrivacyPolicyQuery());
  ...
  if (error || !privacyPolicy?.data) {
    return (
      <div className="min-h-screen max-w-6xl mx-auto bg-white">
        <Container className="py-6 sm:py-8 md:py-12 lg:py-20">
          <div className="text-center">
            <p className="text-red-500">Error loading privacy policy content.</p>
          </div>
        </Container>
      </div>
    );
  ...
  const { title, description } = privacyPolicy.data;
