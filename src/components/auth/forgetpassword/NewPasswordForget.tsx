...
interface NewPasswordForgetProps {
  open: boolean;
...
export default function NewPasswordForget({ 
  open, 
  onOpenChange, 
  token, 
  email, 
  onSuccess 
}: NewPasswordForgetProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setApiError("Şifrə sıfırlanarkən xəta baş verdi. Zəhmət olmasa yenidən cəhd edin.");
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[480px] p-8 bg-white rounded-[2px]">
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-6">
          ...
          >
            {resetPasswordMutation.isPending ? "Şifrə sıfırlanır..." : t("saveButton")}
          </Button>
        </form>
      </DialogContent>
    </Dialog>
