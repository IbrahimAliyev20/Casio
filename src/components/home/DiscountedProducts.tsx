...
import { getDiscountedProductsQuery } from "@/services/product/queries";
import { useLocale } from "next-intl";

export default function DiscountedProducts() {
  const t = useTranslations("home.discountedProducts");
  const locale = useLocale();
  const [api, setApi] = useState<CarouselApi>();
  const [canScrollPrev, setCanScrollPrev] = useState(false);
  const [canScrollNext, setCanScrollNext] = useState(false);

  const { data: discountedProductsData } = useQuery(getDiscountedProductsQuery(locale));

  const discountedProducts: ProductCardData[] = useMemo(() => {
    if (!discountedProductsData?.data) return [];

    return discountedProductsData.data.map((product) => {
